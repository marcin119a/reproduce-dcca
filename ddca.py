...
class CCA(Layer):
    ...
    def __init__(self, output_dim=1, o1=1, o2=1, use_all_singular_values=True, cca_space_dim=10, **kwargs):
    ...
    def call(self, x):
        ...
        H1 = T.transpose(x[:, 0:self.o1])
        H2 = T.transpose(x[:, self.o1:self.o1 + self.o2])
        ...
        SigmaHat11 = partition2 * tf.matmul(H1bar, tf.transpose(H1bar)) + r1 * tf.eye(self.o1)
        SigmaHat22 = partition2 * tf.matmul(H2bar, tf.transpose(H2bar)) + r2 * tf.eye(self.o2)
